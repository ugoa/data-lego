...
/**
  * Duplicated with CommonJDBCService
  *
  * TODO: Refactor with CommonJDBCService
  */
class SapHanaConnector(
  ...
  private val connStr = s"jdbc:sap://$host:$port/?currentschema=$dbSchema&user=$dbUser&password=$dbPassword"

  def getConnection(conn: String = connStr): Connection = {
    try {
      DriverManager.getConnection(conn)
    } catch {
      case ex: SQLException =>
        Logger.error(s"SapHana Connection Failed. ${ex.toString}")
        throw ex
    }
  }
  ...
  def listTables: Seq[String] = {
    ...
    try {
      ...
      val resultSet = metaData.getTables(null, null, "%", null)
  ...
  def getTopRows(tableName: String): Array[Array[(String, Any)]] = {
    ...
    try {
      val query = s"""SELECT * FROM \"$tableName\" LIMIT 10"""
